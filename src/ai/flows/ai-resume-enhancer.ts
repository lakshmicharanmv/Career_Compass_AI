...
const enhanceResumePrompt = ai.definePrompt({
  ...
  4.  **Enhance Skills**: Critically review the technical and soft skills. Correct any misspellings. If the technical skills are sparse or missing, add **no more than 5-6** of the most relevant, industry-standard skills based on the user's professional title, experience, and projects. For example, if a user is a "Software Engineer" but only lists "Java", you could add "Git, SQL, Agile Methodologies, REST APIs". Keep the list concise.
